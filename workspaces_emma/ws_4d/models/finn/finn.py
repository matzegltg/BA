...
class FINN_DiffAD2ss(FINN):
    ...
    This class inherits all parameter from the parent FINN class.
    """
    def __init__(self, u:th.Tensor, D:np.ndarray, BC:np.ndarray, dx, layer_sizes, device, rho_s, f, k_d, beta, n_e, 
    ...
    def flux_kernel(self, t, u):
        ...
        if self.sand:
            ...
            # learn F(c), only functional relation in soil layer. In sand: F(c) = 0
            if not self.learn_f_hyd:
                ...
                f_hyd = th.zeros(self.Nx)
                cw_soil = cw_soil.unsqueeze(-1)
                f_hyd[self.x_start:self.x_stop] = (-self.func_f(cw_soil.float())*th.abs(self.f_fac)).squeeze(1)
                cw_soil = cw_soil.squeeze(1)
                
            # learn R(c)
            if not self.learn_r_hyd:
                ret_hyd = th.ones(self.Nx)
                ret_hyd[self.x_start:self.x_stop] = (f_mod*(k_d_mod*beta_mod*cw_soil**(beta_mod-1))*(self.rho_s/self.n_e))+1
            else:
                # phsics informed: ret > 1 and larger than sigmoid output ([0,1])
                ret_hyd = th.ones(self.Nx)
                cw_soil = cw_soil.unsqueeze(-1)
                ret_hyd[self.x_start:self.x_stop] = (1+self.func_r(cw_soil.float())*(10**self.ret_fac)).squeeze(1)
                cw_soil = cw_soil.squeeze(1)

            # learn G(s_k), only functional relation in soil layer. In sand: G(s_k) = 0, since s_k = 0
            if not self.learn_g_hyd:
                g_hyd = th.zeros(self.Nx)
                g_hyd[self.x_start:self.x_stop] = alpha_mod*self.rho_s/self.n_e*sk_soil
            else:
                g_hyd = th.zeros(self.Nx)
                sk_soil = sk_soil.unsqueeze(-1)
                g_hyd[self.x_start:self.x_stop] = (self.func_g(sk_soil.float())*th.abs(self.g_fac)).squeeze(1)

            # Integrate the fluxes at all boundaries of control volumes i
            flux_c = (top_flux + bot_flux+f_hyd*c+g_hyd)/ret_hyd
