...
def vis_time_dep_func(model):
    #time_points = [2, 3, 4, 5, 6, 20, 40, 80, 120, 150, 180, 200]
    #time_points = th.tensor(time_points)
    concentrations = th.logspace(-5,-2,100)
    #time_points_FINN = time_points.unsqueeze(1).expand(len(time_points),len(concentrations))
    #concentrations_FINN = concentrations.unsqueeze(1).expand(len(concentrations), len(time_points))
    #concentrations_FINN = th.swapaxes(concentrations_FINN, 0, 1)
    sorb_conc = th.logspace(-6,-3,100)
    #sorb_concentrations_FINN = sorb_conc.unsqueeze(1).expand(len(sorb_conc), len(time_points))
    #sorb_concentrations_FINN = th.swapaxes(sorb_concentrations_FINN,0,1)

    r_normal = 1 + model.rho_s/model.n_e * model.f*model.k_d*model.beta*concentrations**(model.beta-1)
    f_normal = -model.rho_s/model.n_e * model.alpha*(1-model.f)*model.k_d*concentrations**(model.beta)
    g_normal = model.rho_s/model.n_e * model.alpha*sorb_conc
    
    #t_c = th.stack((sorb_concentrations_FINN,time_points_FINN), dim=-1)
    concentrations = concentrations.unsqueeze(-1)
    print(concentrations.unsqueeze(-1).shape)
    print((-model.func_f(concentrations.float())).size())
    f_FINN = -model.func_f(concentrations.float())*th.abs(model.f_fac)
    f_FINN = f_FINN.squeeze(-1)
    f_FINN = f_FINN.detach().numpy()
    concentrations = concentrations.squeeze(1)
    concentrations = concentrations.unsqueeze(-1)
    r_FINN = model.func_r(concentrations.float())*10**(model.ret_fac)
    r_FINN = r_FINN.squeeze(-1)
    r_FINN = r_FINN.detach().numpy()
    
    sorb_conc = sorb_conc.unsqueeze(-1)
    g_FINN = model.func_g(sorb_conc.float())*th.abs(model.g_fac)
    g_FINN = g_FINN.squeeze(-1)
    g_FINN = g_FINN.detach().numpy()
    
    font_size = 22
    fig, ax = plt.subplots(1,3)
    
    ax[0].plot(concentrations,r_FINN, label=f"FINN")
    ax[0].plot(concentrations,r_normal,label="2SS")
    ax[0].set_xlabel("$c \left[\\frac{\mu g}{cm^3}\\right]$", fontsize = font_size)
    ax[0].set_ylabel("R(c)", fontsize = font_size)
    ax[0].set_title("R(c)", fontsize=font_size)
    ax[0].legend(fontsize=font_size)
    for label in (ax[0].get_xticklabels() + ax[0].get_yticklabels()): label.set_fontsize(font_size)
    ax[0].set_xscale("log")

    ax[1].plot(concentrations,f_FINN, label=f"FINN")
    ax[1].plot(concentrations,f_normal,label="2SS")
    ax[1].set_xlabel("$c \left[\\frac{\mu g}{cm^3}\\right]$", fontsize = font_size)
    ax[1].set_ylabel("F(c)", fontsize = font_size)
    ax[1].set_title("F(c)", fontsize=font_size)
    ax[1].legend(fontsize=font_size)
    ax[1].set_xscale("log")
    for label in (ax[1].get_xticklabels() + ax[1].get_yticklabels()): label.set_fontsize(font_size)
    
    ax[2].plot(sorb_conc,g_FINN, label=f"FINN")
    ax[2].plot(sorb_conc,g_normal,label="2SS")
    ax[2].set_xlabel("$c \left[\\frac{\mu g}{cm^3}\\right]$", fontsize = font_size)
    ax[2].set_ylabel("G(s_k)", fontsize = font_size)
    ax[2].set_title("G(s_k)", fontsize=font_size)
    ax[2].legend(fontsize=font_size)
    ax[2].set_xscale("log")
    for label in (ax[2].get_xticklabels() + ax[2].get_yticklabels()): label.set_fontsize(font_size)
    fig.set_size_inches(20,10)
    plt.tight_layout()
    plt.savefig(f"results/412/funcs", dpi=500)
