...
def vis_comp_btc(u, u_NN, u_N2, u_NN_N2, t, t_N2, x, exp_mean_t_N2, exp_conc_N2):
    ...
    ax.set_yscale("log")
    ax.scatter(exp_mean_t, exp_conc, color="b", label="Exp. BTC N1")
    ax.scatter(exp_mean_t_N2, exp_conc_N2, color="r", label="Exp. BTC N3")
    ax.plot(t, u_NN[-1,:,0], color="b", label="FINN BTC N1")
    ax.plot(t_N2, u_NN_N2[-1,:,0], color="r", label="FINN BTC N3")
    ...
    plt.tight_layout()
    plt.savefig(f"results/342/btc", dpi=500)
...
def vis_btc(model, u, u_hat, t, x, config_NN:Configuration, exp_mean_t_N2=None, exp_conc_N2=None):
    fig, ax = plt.subplots(1,2)
    font_size = 22
    u_FD = np.load(f"results/345/u_FD.npy")
    ...
    if config_NN.data.name == "data_ext":
        ...
        ax[0].plot(t, u_hat[-1,:,0], color="y", label="FINN pred. BTC")
        ax[0].plot(t, u_FD[-1,:,0], color="g", label="Bierbaum et al. pred. BTC")
    
    # plot sk end
    ax[1].set_xlabel("$s_{tot,end}  \:\left[\\frac{\mu g}{g}\\right]$", fontsize=font_size)
    ax[1].set_ylabel("$x [cm]$", fontsize=font_size)
    t_end = np.round(t[-1],2)
    ax[1].set_title(f"Tot. sorbed conc. of PFOS at t = {t_end}d", fontsize=font_size)
    if config_NN.data.name == "data_ext":
        ax[1].plot(np.flip(u[:,-1,1]), x, color="b", label="FD")
    elif config_NN.data.name == "data_exp":
        ax[1].plot(np.flip(u[:,-1,1]), x,color="b", label="exp.")
    s_tot_hat = np.zeros(u_hat.shape[0])
    s_tot_FD = np.zeros(u.shape[0])
    s_tot_hat[model.x_start:model.x_stop] = u_hat[model.x_start:model.x_stop,-1,1] +1/(1+np.exp(-(model.__dict__['_parameters']['f'].item())))*4.5*u_hat[model.x_start:model.x_stop,-1,0]**0.98
    s_tot_FD[model.x_start:model.x_stop] = u_FD[model.x_start:model.x_stop,-1,1] + 0.93*4.5*u_FD[model.x_start:model.x_stop,-1,0]**0.98
    ax[1].plot(np.flip(s_tot_hat), x, color="y", label="FINN pred.")
    ax[1].plot(np.flip(s_tot_FD), x, color = "g", label="Bierbaum et al. pred.")
...
def vis_sorption_isotherms(model, u, u_FD, u_hat, t, x, config_NN:Configuration):
    font_size = 22
    fig, ax = plt.subplots()
    dt = t[1]-t[0]
    # plot sk over cw
    ...
    for timestep in range(1,len(t)):
        if timestep%modulo_timesteps == 0:
            ...
            if config_NN.data.name == "data_exp":
                ax.plot(u_FD[model.x_start:model.x_stop,timestep,0], u_FD[model.x_start:model.x_stop,timestep,1], "--", label=f"FD: {np.round(timestep*dt,2)}d", color=color, alpha=0.9)
...
def init_model_N2():
    ...
    u_NN_N2 = np.load(f"results/345/u_hat_N2.npy")
    u_N2 = np.load(f"results/345/u_N2.npy")
    t = np.load(f"results/345/t_N2.npy")
    x = np.load(f"results/345/x_series.npy")
...
def vis_data(number):
    ...
    u_FD = np.load("results/345/u_FD.npy")
    ...
    vis_sorption_isotherms(model, u, u_FD, u_NN, t, x, config_NN)
