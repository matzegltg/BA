...
def run_testing(print_progress=False, visualize=False, model_number=None):    
    ...
    params_N1 = Configuration(f"results/37/init_params.json")
    ...
    last_sk[params_N1.sand.top:params_N1.sand.bot] = params_N2.kin_sorb_end
    sample_sk[:,-1]= last_sk
    ...
    with open(f"results/37/model.pkl", "rb") as inp:
    ...
    for eval_index in loss_indices:
        loss_1 += ((th.log(u_hat[-1,eval_index,0]+1)-th.log(u[-1,eval_index,0]+1)))**2
    loss_1 = loss_1/len(loss_indices)
    ...
    np.save(f"results/37/u_N2", u)
    np.save(f"results/37/u_hat_N2", u_hat)
    np.save(f"results/37/t_N2", t)
    return model
